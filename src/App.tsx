...
const getAi = () => {
  // Safely attempt to read the key without triggering a "process is not defined" exception in the browser
  let key = import.meta.env.VITE_GEMINI_API_KEY;
  if (!key && typeof process !== 'undefined' && process.env) {
    key = process.env.GEMINI_API_KEY;
  }
  // Fallback for when Vite replaces the variable directly
  if (!key) {
    try {
      key = process.env.GEMINI_API_KEY;
    } catch (e) {
      // Ignored: process was not defined and Vite didn't replace it
    }
  }
